...
//     pa0 = walkaddr(pagetable, va0);  // 用户页表的物理地址转换方式
//     if(pa0 == 0)
//       return -1;
//     n = PGSIZE - (srcva - va0);      // 计算当前页面剩余的字节数
//     if(n > len)
//       n = len;
//     memmove(dst, (void *)(pa0 + (srcva - va0)), n);  // 只复制不超过 当前页面剩余可字节数的
...
//     char *p = (char *) (pa0 + (srcva - va0));  // 根据偏移找到对应的物理地址
//     while(n > 0){                              // 遍历字符    
//       if(*p == '\0'){
//         *dst = '\0';
//         got_null = 1;
...
u2kvmcopy(pagetable_t upgtbl, pagetable_t kpgtbl, uint64 oldsz, uint64 newsz)
{
  ...
  for(i = PGROUNDUP(oldsz); i <  newsz ; i += PGSIZE )
  {
    ...
    if((pte_to = walk(kpgtbl, i, 1)) == 0)
      panic("u2kvmcopy: page not exist");
